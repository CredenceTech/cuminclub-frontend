...
function DropdownFilter({
  ...
  return (
    <div className="relative inline-flex">
      ...
      >
        <p className='text-[#231F20] text-xl font-skillet '>{selectedMealData?.noMeal} <span className='text-[#279C66]'>{selectedMealData?.price}</span></p>
          ...
          <ul className="m-2">
            {data?.map((item) => (
              <li onClick={() => { dispatch(addMeal(item)) }} key={item?.id} className={`py-1 px-3 cursor-pointer ${selectedMealData?.id === item?.id ? 'opacity-50' : ''} `}>
                <p className='text-[#231F20] text-xl font-skillet '>{item?.noMeal} <span className='text-[#279C66]'>{item?.price}</span></p>
              </li>
            ))}
          </ul>
        </div>
